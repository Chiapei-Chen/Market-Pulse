...
watch(
  () => [rankingStore.includeEtf, rankingStore.topN],
  () => {
    void rankingStore.loadRankings()
  },
)
...
<template>
  <main class="min-h-screen bg-[radial-gradient(circle_at_15%_20%,#fee2e2_0%,transparent_25%),radial-gradient(circle_at_80%_10%,#dbeafe_0%,transparent_30%),linear-gradient(145deg,#f8fafc,#eef2ff)] px-4 py-8 text-slate-800 sm:px-8">
    <section class="mx-auto max-w-6xl">
      ...
      <section class="mb-6 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
      ...
      <RankingTable v-if="rankingStore.rows.length > 0" :rows="rankingStore.rows" />
    </section>
  </main>
</template>
