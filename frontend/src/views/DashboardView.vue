...
const matchedTags = computed(() => {
  ...
  return availableTags.value.filter((tag) => tag.toLowerCase().includes(keyword.toLowerCase()))
})
...
const matchedIndustries = computed(() => {
  ...
  return availableIndustries.value.filter((industry) => industry.toLowerCase().includes(keyword.toLowerCase()))
})
...
const filteredRows = computed(() => {
  return rankingStore.rows.filter((row) => {
    ...
    const passIndustry = !selectedIndustry.value || row.industry_level_1 === selectedIndustry.value
...
async function handleAddTag(payload: { symbol: string; industry: string; tag: string }) {
    ...
    <section class="mx-auto max-w-6xl">
      ...
      <nav class="mb-6 flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">
        <button
          ...
          :class="activeSection === 'ranking' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'"
        ...
        <button
          type="button"
          class="rounded-xl px-4 py-2 text-sm font-semibold transition"
          :class="activeSection === 'tag-manager' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'"
      ...
      <section v-if="activeSection === 'ranking'" class="mb-6 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-start sm:justify-between">
        <div class="grid gap-3 sm:grid-cols-2 sm:gap-6">
          ...
          <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            <p class="mb-2 text-xs font-bold uppercase tracking-wider text-slate-500">標籤查詢</p>
            <div class="relative w-64">
              <div class="flex items-center gap-2">
                <input
                  ...
                  @keydown.enter.prevent="matchedTags.length > 0 ? applyTagFilter(matchedTags[0]) : clearTagFilter()"
              ...
              >
                ...
                <li v-if="matchedTags.length === 0" class="px-3 py-2 text-sm text-slate-400">
                  找不到符合的標籤
                </li>
              </ul>
            </div>
            <p class="mt-2 text-xs text-slate-500">
              下拉視窗一次顯示約 5 筆，可用滾輪上下滑動；目前篩選：{{ selectedTag || '全部標籤' }}
            </p>
          </div>

          <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            <p class="mb-2 text-xs font-bold uppercase tracking-wider text-slate-500">產業查詢</p>
            <div class="relative w-64">
              <div class="flex items-center gap-2">
                <input
                  ...
                  @keydown.enter.prevent="matchedIndustries.length > 0 ? applyIndustryFilter(matchedIndustries[0]) : clearIndustryFilter()"
              ...
              >
                ...
                <li v-if="matchedIndustries.length === 0" class="px-3 py-2 text-sm text-slate-400">
                  找不到符合的產業
                </li>
              </ul>
            </div>
            <p class="mt-2 text-xs text-slate-500">
              下拉視窗一次顯示約 5 筆，可用滾輪上下滑動；目前篩選：{{ selectedIndustry || '全部產業' }}
            </p>
          </div>
        </div>
      ...
      <section v-if="activeSection === 'ranking'" class="mb-6 grid gap-4 lg:grid-cols-2">
      ...
      <p v-if="!rankingStore.loading && !rankingStore.error && filteredRows.length === 0" class="mb-4 rounded-lg bg-slate-100 px-4 py-2 text-sm font-medium text-slate-700">目前沒有符合條件的資料。</p>
